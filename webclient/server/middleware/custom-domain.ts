...
async function fetchCustomDomain(host: string): Promise<{ slug: string; locale?: string | null } | null> {
  // Check cache first
  const now = Date.now();
  ...
  try {
    ...
    const response = await fetch(`${backendUri}/custom-domains/${encodeURIComponent(host)}`);
    ...
    console.error(`[custom-domain] Failed to lookup custom domain: ${host}`, error);
    domainCache.set(host, null);
    cacheTimestamps.set(host, now);
    return null;
  }

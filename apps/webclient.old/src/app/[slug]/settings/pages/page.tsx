...
export default function ProfilePagesPage() {
  ...
  const handleCreatePage = async () => {
    if (!createForm.slug || !createForm.title || !createForm.summary) {
      setError(
        t("Profile", "Please fill in all required fields") ??
    ...
    try {
      ...
      setError(
        t("Profile", "Failed to create page") ?? "Failed to create page",
      );
    } finally {
  ...
  const handleDeletePage = async (pageId: string) => {
    try {
      ...
      setError(
        t("Profile", "Failed to delete page") ?? "Failed to delete page",
      );
    }
  };
    ...
    return (
      <div className={styles.container}>
        <div className={styles.error}>
          {t("Profile", "Please log in to manage pages") ??
            "Please log in to manage pages"}
        </div>
      </div>
    );
  }
      ...
      <div className={styles.actions}>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button>
              {t("Profile", "Create New Page") ?? "Create New Page"}
            </Button>
          </DialogTrigger>
          <DialogContent className={styles.dialog}>
            ...
            <div className={styles.form}>
              <div className={styles.field}>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      slug: e.target.value.toLowerCase().replace(
                        /[^a-z0-9-]/g,
                        "-",
                      ),
                    })}
                  placeholder={t("Profile", "my-custom-page") ??
                    "my-custom-page"}
                  className={styles.input}
                />
                <p className={styles.hint}>
                  {t("Profile", "URL-friendly identifier for your page") ??
                    "URL-friendly identifier for your page"}
                </p>
              </div>
              <div className={styles.field}>
                <Label htmlFor="page-title">
                  {t("Profile", "Page Title") ?? "Page Title"} *
                </Label>
                <Input
                  id="page-title"
                  value={createForm.title}
                  onChange={(e) => setCreateForm({ ...createForm, title: e.target.value })}
                  placeholder={t("Profile", "Enter page title") ??
                    "Enter page title"}
                  className={styles.input}
                />
              </div>
              <div className={styles.field}>
                <Label htmlFor="page-summary">
                  {t("Profile", "Page Summary") ?? "Page Summary"} *
                </Label>
                <Textarea
                  id="page-summary"
                  value={createForm.summary}
                  onChange={(e) => setCreateForm({ ...createForm, summary: e.target.value })}
                  placeholder={t("Profile", "Brief description of your page") ??
              ...
              <div className={styles.field}>
                <Label htmlFor="page-content">
                  {t("Profile", "Page Content") ?? "Page Content"}
                </Label>
                <Textarea
                  id="page-content"
              ...
              <div className={styles.field}>
                <Label htmlFor="page-cover">
                  {t("Profile", "Cover Image URL") ?? "Cover Image URL"}
                </Label>
                <Input
                  id="page-cover"
                  ...
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      cover_picture_uri: e.target.value,
                    })}
                  placeholder={t("Profile", "https://example.com/image.jpg") ??
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={isSubmitting || !createForm.slug ||
                  !createForm.title || !createForm.summary}
              >
                {isSubmitting
                ...
                <CardHeader>
                  <div className={styles.pageHeader}>
                    <div className={styles.pageInfo}>
                      ...
                      <Badge variant="secondary" className={styles.pageSlug}>
                        /{page.slug}
                      </Badge>
                    </div>

...
export default function ProfileTranslationsPage() {
      ...
      try {
        ...
        if (profileData) {
          ...
          if (isAuthenticated) {
            ...
            const translationsResponse = await backend.getProfileTranslations(
              localeCode,
              slug,
            );
            if (translationsResponse) {
              const translationsMap = new Map<string, ProfileTranslation>();
              translationsResponse.forEach(
  ...
  const onSubmit = async (data: TranslationFormData) => {
    ...
    try {
      ...
      setError(
        t("Profile", "Failed to update translation") ??
  ...
  const copyFromLocale = (sourceLocale: string) => {
    ...
    if (sourceTranslation) {
      ...
      setError(
        t("Profile", "Failed to copy content") || "Failed to copy content",
      );
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {t("Profile", "Available Languages") || "Available Languages"}
          </CardTitle>
          <CardDescription>
            {t(
              "Profile",
          ...
          {availableSourceLocales.length > 0 && (
            <div className={styles.copySection}>
              <label className={styles.copyLabel}>
                {t("Profile", "Copy from existing translation:") ||
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder={t(
                          "Profile",
                          "Profile title in this language",
                        ) || "Profile title in this language"}
                        {...field}
                      />
                    </FormControl>

...
export function CreateProfileForm() {
  ...
  const onSubmit = async (data: CreateProfileFormData) => {
    ...
    if (data.kind === "individual" && hasIndividualProfile) {
      setError(
        "You already have an individual profile. Please select a different profile type.",
      );
      return;
    }
                    ...
                    <div className="relative">
                      ...
                      {watchedSlug.length >= 3 && (
                        <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                          ...
                          {!slugAvailability.isChecking &&
                            slugAvailability.isAvailable === true && <span className="text-green-500 text-sm">✓</span>}
                          {!slugAvailability.isChecking &&
                            slugAvailability.isAvailable === false && <span className="text-red-500 text-sm">✗</span>}
                        </div>
                      )}
                    </div>
                  </FormControl>
                    ...
                    {watchedSlug.length >= 3 && !slugAvailability.isChecking &&
                      ...
                      (
                        <span className="text-green-600">
                          ✓ Available! Your profile will be at: aya.is/{watchedSlug}
                        </span>
                      )}
                    {watchedSlug.length >= 3 && !slugAvailability.isChecking &&
                      slugAvailability.isAvailable === false && (
                      <span className="text-red-600">
                        ✗ {slugAvailability.error ?? "This slug is not available"}
                      </span>
                    )}
                    {watchedSlug.length >= 3 && !slugAvailability.isChecking &&
                      slugAvailability.isAvailable === null &&
                      (
                        <span className="text-gray-600">
                          Your profile will be available at: aya.is/{watchedSlug}
                        </span>
                      )}
                    {watchedSlug.length < 3 &&
                      "This will be used in your profile URL (e.g., aya.is/your-profile-name)"}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}

...
async function applyCustomDomainMiddleware(
  req: NextRequest,
  res: NextResponse,
  ...
  const target = new URL(
    `/${customDomain.slug}${req.nextUrl.pathname}`,
    req.nextUrl,
  );
...
function applyLocaleMiddleware(
  req: NextRequest,
  res: NextResponse,
): NextResponse {
  const localeState = localeMatcher.localeMatchFromRequest(
    req,

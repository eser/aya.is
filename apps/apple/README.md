...
Traditional **MVVM** (Model-View-ViewModel) with a flat single-package structure:
...
- **Direct instantiation** — no dependency injection container; ViewModels receive concrete dependencies via init
...
- **Localization** — English and Turkish with `String(localized:)` and `.strings` files
...
- **Language switching** — English/Turkish with live reload
...
Strings files are in `Resources/{platform}/{locale}.lproj/Localizable.strings`. Currently supported:
